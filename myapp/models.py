...
#         # Check item availability for each item in the order
#         order_items = self.salesorder_items.all()  # Assuming you have a related field for order items
#         for order_item in order_items:
#             stock_item = StockItems.objects.filter(item=order_item.item).first()
#             if not stock_item or stock_item.quantity < order_item.quantity:
#                 raise ValidationError(f"Item {order_item.item.item_name} is not available in sufficient quantity.")
